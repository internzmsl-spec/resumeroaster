...
import { Upload, Sparkles, TrendingUp, CheckCircle, XCircle, AlertCircle, Mail, Calendar, BookOpen, Loader2 } from 'lucide-react';

export default function ResumeRoaster() {
  const [step, setStep] = useState('landing'); // landing, setup, analyzing, results
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      if (text.length < 200) {
        setError('This seems too short to be a resume. Please upload a complete resume.');
        setLoading(false);
        return;
      }
      setResumeText(text);
  ...
  // Simple PDF text extraction (in production, use pdf.js)
  const extractTextFromPDF = async (file) => {
    return new Promise((resolve, reject) => {
      const reader = new FileReader();
      reader.onload = async (e) => {
        // This is a simplified version - in production use pdf.js
        // For now, we'll just show a message to paste text
        reject('PDF parsing requires pdf.js library');
      };
...
Be witty, memorable, and valuable. Make them WANT the full detailed analysis.`
  ...
  const parseAnalysis = (text) => {
    ...
    const redFlagsMatch = text.match(/# 🚩 RED FLAGS\n([\s\S]*?)$/);
      ...
      <main className="max-w-6xl mx-auto px-4 py-12">
        ...
        {step === 'results' && analysis && (
          <div className="space-y-8">
            ...
            {/* Show remaining sections after email capture */}
            {emailCaptured && (
              <>
                {/* Quick Wins */}
